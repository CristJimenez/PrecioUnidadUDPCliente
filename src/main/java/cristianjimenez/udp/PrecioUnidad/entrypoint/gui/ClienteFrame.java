...
import cristianjimenez.udp.PrecioUnidad.adaptadores.notificacion.ObservadorCliente;
...
public final class ClienteFrame extends JFrame implements ObservadorCliente {
    ...
    private final transient CalcularPrecioUnidadcInputPort calculo;
    ...
    private final JButton calcular = new JButton("Calcular Precio por Unidad");
    private final JLabel estado = new JLabel("● DESCONECTADO");
    private final JLabel resultado = new JLabel("Precio por Unidad: --");
    private final JTextArea logs = new JTextArea();

    public ClienteFrame(final GestionarConexionInputPort conexion, final CalcularPrecioUnidadcInputPort calculo) {
        super("Cliente UDP - Cálculo de Precio por Unidad (Arquitectura Hexagonal)");
    ...
    private void solicitarCalculo() {
        try {
            final double valorPrecio = Double.parseDouble(precio.getText().trim().replace(',', '.'));
            final int valorCantidad = Integer.parseInt(cantidad.getText().trim().replace(',', '.'));
            calcular.setEnabled(false);
            calculo.calcular(new CalcularPrecioUnidadCommand(valorPrecio, valorCantidad));
        } catch (final NumberFormatException | DominioException excepcion) {
            mostrarError(excepcion.getMessage());
        }
    }
    ...
    @Override
    public void onResultado(final ResultadoPrecioUnidad valor) {
        SwingUtilities.invokeLater(() -> {
            resultado.setText("Precio por  Unidad: " + valor.precioUnidad());
    ...
    private void habilitarCalculo(final boolean habilitado) {
        precio.setEnabled(habilitado);
        cantidad.setEnabled(habilitado);
